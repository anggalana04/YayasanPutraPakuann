...
<div class="p-8 max-w-7xl mx-auto w-full space-y-6">
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
            <h2 class="text-3xl font-bold tracking-tight text-[#1c190d]">Manajemen Kapasitas</h2>
            <p class="text-on-surface-variant">Sekolah: {{ $school->name }} | Tahun Ajaran: {{ $year }}</p>
        </div>
        <a href="{{ route('admin.ppdb.management', ['school' => $school->slug, 'year' => $year]) }}" class="px-5 py-2 rounded-xl bg-primary text-on-primary font-bold hover:bg-primary/90">Kembali ke Manajemen PPDB</a>
    </div>
    ...
    <div class="bg-surface-container-lowest rounded-3xl p-6 border border-outline">
        <h3 class="text-xl font-bold mb-3">Tambahkan / Perbarui Kapasitas Jurusan</h3>
        <form method="POST" action="{{ route('admin.ppdb.management.capacity.store', ['school' => $school->slug]) }}" class="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
            ...
            <div>
                <label class="block text-xs font-bold uppercase text-on-surface-variant mb-1">Jurusan</label>
    ...
    <div class="bg-surface-container-lowest rounded-3xl p-6 border border-outline">
        <h3 class="text-xl font-bold mb-4">Daftar Kapasitas Jurusan</h3>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-outline">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-4 px-3 text-center text-on-surface-variant">Belum ada data kapasitas jurusan.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
...
function confirmDeleteCapacity(id, school, year) {
    if (!confirm('Hapus kapasitas jurusan ini?')) {
        return;
    }
