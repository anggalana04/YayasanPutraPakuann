...
<span id="totalApplicantsValue" class="text-3xl font-black text-[#f2cc0d]">{{ $totalApplicantsCount }}</span>
...
<div class="flex items-end justify-between">
<span id="pendingApplicantsValue" class="text-3xl font-black text-[#1c190d]">{{ $pendingReviewCount }}</span>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-outline-variant/30">
                    ...
                    <th class="px-4 py-4 text-left text-xs font-bold text-on-surface-variant uppercase tracking-wider">Pendaftaran Step</th>
                    <th class="px-4 py-4 text-left text-xs font-bold text-on-surface-variant uppercase tracking-wider">Status Pembayaran</th>
            ...
            <tbody id="applicantTableBody" class="divide-y divide-outline-variant/20">
                ...
                <tr class="hover:bg-surface-container-low/50 transition-colors group">
                    ...
                    <td class="px-4 py-3 text-on-surface-variant text-xs">
                        @php
                            if (in_array($applicant->status, ['accepted', 'accepted_major_1', 'accepted_major_2'])) {
                                $registrationStepLabel = 'Diterima';
                            } elseif ($applicant->status === 'rejected') {
                                $registrationStepLabel = 'Ditolak';
                            } elseif ($applicant->interview_date) {
                                $registrationStepLabel = 'Wawancara';
                            } elseif ($applicant->last_registration_step === 'jurusan_berkas') {
                                $registrationStepLabel = 'Berkas';
                            } elseif ($applicant->last_registration_step === 'done') {
                                $registrationStepLabel = 'Selesai';
                            } else {
                                $registrationStepLabel = 'Biodata';
                            }
                        @endphp
                    ...
                    <td class="px-4 py-3">
                        @php
                            if (in_array($applicant->status, ['pending', 'payment_uploaded'])) {
                                $paymentStatusLabel = 'Belum Bayar';
                                $paymentStatusClass = 'bg-warning-container text-on-warning-container';
                            } elseif (in_array($applicant->status, ['payment_confirmed', 'accepted', 'accepted_major_1', 'accepted_major_2'])) {
                                $paymentStatusLabel = 'Sudah Bayar';
                                $paymentStatusClass = 'bg-success-container text-on-success-container';
                            } elseif ($applicant->status === 'rejected') {
                                $paymentStatusLabel = 'Ditolak';
                                $paymentStatusClass = 'bg-error-container text-on-error-container';
                            } else {
                                $paymentStatusLabel = ucfirst(str_replace('_', ' ', $applicant->status));
                                $paymentStatusClass = 'bg-surface-container text-on-surface';
                            }
                        @endphp
                        <span class="text-xs font-bold px-2.5 py-1 rounded-full {{ $paymentStatusClass }}">{{ $paymentStatusLabel }}</span>
                    </td>
    ...
    (() => {
        ...
        function renderApplicantRows(applicants, page = 1) {
            if (!applicants.length) {
                applicantTableBody.innerHTML = '<tr><td colspan="8" class="px-4 py-8 text-center text-on-surface-variant">Tidak ada pendaftar ditemukan untuk filter yang dipilih.</td></tr>';
                return;
            }
            ...
            applicantTableBody.innerHTML = applicants.map((app, idx) => {
                const statusClass = ['pending', 'payment_uploaded'].includes(app.status)
                    ...
                    : (['payment_confirmed', 'accepted', 'accepted_major_1', 'accepted_major_2'].includes(app.status)
                        ? 'bg-success-container text-on-success-container'
                        : (app.status === 'rejected' ? 'bg-error-container text-on-error-container' : 'bg-surface-container text-on-surface'));
                const paymentStatusLabel = app.payment_status || (['pending', 'payment_uploaded'].includes(app.status)
                    ? 'Belum Bayar'
                    : (['payment_confirmed', 'accepted', 'accepted_major_1', 'accepted_major_2'].includes(app.status)
                        ? 'Sudah Bayar'
                        : (app.status === 'rejected' ? 'Ditolak' : (app.status ? app.status.charAt(0).toUpperCase() + app.status.slice(1) : '—'))));
                ...
                return `
                    <tr class="hover:bg-surface-container-low/50 transition-colors group">
                        ...
                        <td class="px-4 py-3 text-on-surface-variant">${app.registration_step || 'Biodata'}</td>
                        <td class="px-4 py-3"><span class="text-xs font-bold px-2.5 py-1 rounded-full ${statusClass}">${paymentStatusLabel}</span></td>
                        <td class="px-4 py-3 text-on-surface-variant text-xs">${app.payment_method ? app.payment_method.replace(/_/g, ' ').toUpperCase() : '—'}</td>
        ...
        async function loadFilteredData(page = 1) {
            ...
            if (paymentStatusFilter.value && paymentStatusFilter.value !== 'all') params.append('payment_status', paymentStatusFilter.value);
