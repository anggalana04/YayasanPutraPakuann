...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
    @php
        $acceptedCount = \App\Models\PpdbApplication::where('school_type', $schoolModel->type)
            ->where('status', 'accepted')
            ->count();
        $rejectedCount = \App\Models\PpdbApplication::where('school_type', $schoolModel->type)
            ->where('status', 'rejected')
            ->count();
    @endphp
    <div class="bg-green-50 shadow border border-green-200 rounded-2xl p-4">
        <div class="flex justify-between items-start gap-2">
            <div>
                <p class="text-xs text-green-700 uppercase tracking-widest">Diterima</p>
                <h5 class="text-3xl font-bold text-green-800 mt-2">{{ $acceptedCount }}</h5>
            </div>
            <span class="material-symbols-outlined text-green-600 text-4xl">check_circle</span>
        </div>
    </div>
    <div class="bg-red-50 shadow border border-red-200 rounded-2xl p-4">
        <div class="flex justify-between items-start gap-2">
            <div>
                <p class="text-xs text-red-700 uppercase tracking-widest">Ditolak</p>
                <h5 class="text-3xl font-bold text-red-800 mt-2">{{ $rejectedCount }}</h5>
            </div>
            <span class="material-symbols-outlined text-red-600 text-4xl">cancel</span>
        </div>
    </div>
    <div class="bg-blue-50 shadow border border-blue-200 rounded-2xl p-4">
        <div class="flex justify-between items-start gap-2">
            <div>
                <p class="text-xs text-blue-700 uppercase tracking-widest">Menunggu Review</p>
                <h5 class="text-3xl font-bold text-blue-800 mt-2">{{ $applicants->whereIn('status', ['payment_uploaded', 'pending'])->count() }}</h5>
            </div>
            <span class="material-symbols-outlined text-blue-600 text-4xl">pending_actions</span>
        </div>
    </div>
</div>
            ...
            <tbody id="applicantTableBody">
                ...
                <tr class="bg-surface-container-low rounded-2xl">
                    ...
                    <td class="py-3 px-3 rounded-r-2xl">
                        <a href="{{ route('admin.ppdb.applicants.by_school.detail', ['school' => $schoolModel->slug, 'id' => $applicant->id]) }}" class="inline-flex items-center px-4 py-2 bg-primary text-on-primary rounded-2xl text-xs font-bold hover:bg-primary/90 transition-colors">
            ...
            applicantTableBody.innerHTML = applicants.map((app, idx) => {
                ...
                return `
                    <tr class="bg-surface-container-low rounded-2xl">
                        ...
                        <td class="py-3 px-3 rounded-r-2xl">
                            <a href="{{ url('/admin/ppdb/applicants/' . $schoolModel->slug) }}/${app.id}" class="inline-flex items-center px-4 py-2 bg-primary text-on-primary rounded-2xl text-xs font-bold hover:bg-primary/90 transition-colors">
                                <span class="material-symbols-outlined text-sm mr-1">visibility</span> Detail
                            </a>
                        </td>
                    </tr>
