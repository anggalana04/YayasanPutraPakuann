...
@section('title', ($mode === 'create' ? 'Tambah' : 'Ubah') . ' Gambar Carousel - ' . strtoupper($schoolType))

@section('content')
<div class="p-10 max-w-4xl mx-auto space-y-6">
    <div class="mb-6 flex justify-between items-end">
        <div>
            <p class="text-primary font-bold tracking-widest text-xs uppercase">{{ $mode === 'create' ? 'Tambah' : 'Ubah' }} Gambar Carousel</p>
    ...
    <form method="POST" action="{{ $mode === 'create' ? route('admin.cms.carousel.store', ['schoolType' => $schoolType]) : route('admin.cms.carousel.update', ['schoolType' => $schoolType, 'carousel' => $item->id]) }}" enctype="multipart/form-data">
        ...
        <div class="bg-surface-container-lowest rounded-3xl p-6 shadow-sm ring-1 ring-[#1c190d]/5 space-y-6">
            <div class="space-y-2">
                <label class="text-xs font-bold uppercase">Judul Slide (opsional)</label>
                <input type="text" name="title" value="{{ old('title', $item->title ?? '') }}" class="w-full border rounded-xl p-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" placeholder="Contoh: Selamat Datang" maxlength="100" />

                <label class="text-xs font-bold uppercase">Deskripsi Slide (opsional)</label>
                <textarea name="description" rows="2" class="w-full border rounded-xl p-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" maxlength="350" placeholder="Contoh: Pilih program keahlian terbaik di SMK Putra Pakuan">{{ old('description', $item->description ?? '') }}</textarea>
                ...
                @if ($mode === 'create')
                    <label class="text-xs font-bold uppercase">Unggah Gambar (opsional)</label>
