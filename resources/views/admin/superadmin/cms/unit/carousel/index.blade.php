...
@php
    ...
    $pageTitle = $isFacilityMode ? 'CMS Fasilitas - ' . strtoupper($schoolType) : 'CMS Carousel - ' . strtoupper($schoolType);
@endphp
...
<div class="p-10 max-w-7xl mx-auto space-y-6">
    <div class="flex justify-between items-end gap-4">
        <div>
            <p class="text-primary font-bold tracking-widest text-xs uppercase">{{ $isFacilityMode ? 'Facility Management' : 'Carousel Management' }}</p>
            <h2 class="text-4xl font-extrabold tracking-tight text-[#1c190d]">{{ strtoupper($schoolType) }}</h2>
            <p class="text-on-surface-variant max-w-2xl">{{ $isFacilityMode ? 'Kelola fasilitas Yayasan yang tampil pada halaman fasilitas publik.' : 'Kelola gambar carousel pada homepage sekolah ini.' }}</p>
        </div>
        <div class="flex gap-3">
            ...
            <a href="{{ route('admin.cms.carousel.create', ['schoolType' => $schoolType]) }}"
               class="px-6 py-3 bg-primary text-on-primary font-bold rounded-2xl shadow-lg hover:shadow-xl active:scale-95 transition-all text-sm">
                {{ $isFacilityMode ? '+ Tambah Fasilitas' : '+ Tambah Gambar Carousel' }}
            </a>
        </div>
    </div>
    ...
    <div class="bg-surface-container-lowest rounded-3xl p-5 shadow-sm ring-1 ring-[#1c190d]/5 overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-[#1c190d]/5">
                @forelse ($carouselImages as $item)
                    <tr class="hover:bg-surface-container-low/30 transition-colors">
                        <td class="px-4 py-4 space-y-2">
                            ...
                            @if (! $isFacilityMode && $item->video_url)
                            ...
                            @elseif($item->image_url)
                                <img src="{{ $item->image_url }}" alt="{{ $isFacilityMode ? 'Fasilitas' : 'Carousel' }}" class="w-32 h-20 object-cover rounded-xl border border-[#1c190d]/10 bg-white" />
                            @else
                        ...
                        <td class="px-4 py-4 text-right">
                            <div class="flex justify-end gap-2 items-center">
                                ...
                                <form method="POST" action="{{ route('admin.cms.carousel.destroy', ['schoolType' => $schoolType, 'carousel' => $item->id]) }}" onsubmit="return confirm('{{ $isFacilityMode ? 'Hapus fasilitas ini?' : 'Hapus gambar carousel ini?' }}');">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-8 text-center text-on-surface-variant">{{ $isFacilityMode ? 'Belum ada fasilitas. Tambahkan lewat tombol atas.' : 'Belum ada gambar carousel. Tambahkan lewat tombol atas.' }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
