...
@php
    $articleSchema = [
        ...
        'image' => [$newsImage],
        'datePublished' => optional($news->published_at ?? $news->created_at)->toAtomString(),
        'dateModified' => optional($news->updated_at ?? $news->published_at ?? $news->created_at)->toAtomString(),
        'mainEntityOfPage' => [
            '@type' => 'WebPage',
            '@id' => $newsUrl,
        ],
...
@section('content')
@php
    $imageRaw = trim((string)($news->image_url ?? ''));
    ...
    $dateStr    = $news->published_at
        ? \Illuminate\Support\Carbon::parse($news->published_at)->translatedFormat('d F Y')
        : '';
@endphp
...
<div class="w-full max-w-[860px] mx-auto px-4 sm:px-6 pt-6">
    <div class="flex flex-wrap items-center gap-1.5 text-xs font-medium text-slate-500 dark:text-slate-400">
        ...
        <a class="hover:text-slate-900 dark:hover:text-white transition-colors" href="{{ route('yayasan.berita') }}">Berita</a>
        <span class="text-slate-300 dark:text-slate-600">/</span>
        <span class="text-slate-700 dark:text-slate-300 line-clamp-1 max-w-[240px]">{{ $news->title }}</span>
    </div>
</div>
...
<article class="w-full max-w-[860px] mx-auto px-4 sm:px-6 py-8 pb-20">
    ...
    @if (!$imageUrl)
        <div class="mb-8">
            ...
            <h1 class="text-3xl md:text-4xl font-black text-slate-900 dark:text-white leading-tight">{{ $news->title }}</h1>
        </div>
    @else
        {{-- Repeat title below hero for SEO / skability --}}
        <div class="mb-6 pt-2">
            @if (!$dateStr)
            @endif
        </div>
