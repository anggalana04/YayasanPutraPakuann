{{--
    Shared partial: unit (SD / SMP / SMK) prestasi detail.
    Expected variables injected by the calling view:
        $school      – slug string  (sd | smp | smk)
        $schoolModel – App\Models\School instance
        $prestasi    – App\Models\Prestasi instance
        $related     – Illuminate\Support\Collection of Prestasi
--}}
@php
    use Illuminate\Support\Carbon;
    use Illuminate\Support\Str;

    $metaExcerpt     = trim(strip_tags((string)($prestasi->excerpt ?? $prestasi->content ?? 'Prestasi unggulan ' . ($schoolModel->name ?? 'Sekolah') . '.')));
    ...
    $prestasiUrl     = route('school.prestasi.show', ['school' => $school, 'slug' => $prestasi->slug]);
    $prestasiListUrl = route('school.prestasi', ['school' => $school]);
    ...
    $schoolLabel = strtoupper($schoolModel->name ?? strtoupper($school));
@endphp
...
<div class="w-full relative overflow-hidden" style="height: clamp(240px, 40vw, 480px);">
    ...
    <div class="relative z-10 h-full max-w-[900px] mx-auto px-4 sm:px-6 flex flex-col justify-end pb-10 md:pb-14">
        ...
        <nav class="flex items-center gap-2 text-white/60 text-xs font-medium mb-4">
            <a href="{{ route('school.home', ['school' => $school]) }}" class="hover:text-white transition-colors">Beranda</a>
            <span>/</span>
            <a href="{{ $prestasiListUrl }}" class="hover:text-white transition-colors">Prestasi</a>
            <span>/</span>
...
<div class="max-w-[1100px] mx-auto px-4 sm:px-6 py-12 grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-10">
    ...
    <article>
        ...
        <div class="mt-12 pt-6 border-t border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                ...
                <button onclick="navigator.clipboard.writeText('{{ $prestasiUrl }}').then(function(){ this.innerHTML='<span class=\'material-symbols-outlined text-[16px]\'>check</span>'; var btn=this; setTimeout(function(){ btn.innerHTML='<span class=\'material-symbols-outlined text-[16px]\'>link</span>'; },1800); }.bind(this))"
    ...
    <aside class="space-y-6">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 space-y-4 shadow-sm">
            ...
            <div class="flex items-start gap-3">
                ...
                <div>
                    ...
                    <p class="font-bold text-slate-900 dark:text-white">{{ $schoolModel->name ?? $schoolLabel }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-[#FDB913]/10 border border-[#FDB913]/30 rounded-2xl p-6 text-center">
            ...
            <p class="text-xs text-slate-600 dark:text-slate-400 mb-4">Raih prestasi terbaik bersama {{ $schoolModel->name ?? $schoolLabel }}</p>
            <a href="{{ route('school.kontak', ['school' => $school]) }}"
               ...
               class="group flex flex-col bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                ...
                <div class="p-5 flex flex-col flex-1">
                    <div class="flex justify-between items-start mb-2">
                        <span class="text-xs font-bold text-[#FDB913]">{{ $schoolLabel }}</span>
