...
<head>
    ...
    <title>@yield('title', 'SDIT Putra Pakuan')</title>
    ...
    <script id="tailwind-config">
        tailwind.config = {
            ...
            theme: {
                extend: {
                    colors: {
                        primary: "#16a34a", // SD green (emerald-600)
...
<body class="bg-background-light dark:bg-background-dark text-charcoal dark:text-slate-100">
    ...
    <nav class="sticky top-0 z-50 w-full bg-charcoal text-white border-b border-primary/20">
        <div class="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-[10px] text-primary font-medium tracking-widest uppercase">Islamic Primary School</p>
                </div>
            </div>
            <div class="hidden lg:flex items-center gap-6">
                ...
                <button class="bg-primary hover:bg-primary/90 text-charcoal px-5 py-2.5 rounded-lg text-xs font-bold transition-all ml-4" onclick="window.location.href='{{ route('school.ppdb', ['school' => 'sd']) }}'">
                    PPDB 2024
                </button>
            </div>
            <!-- Mobile menu button -->
        ...
        <div id="mobile-menu" class="fixed inset-0 z-50 bg-charcoal/95 text-white flex flex-col gap-6 px-8 py-10 transition-all duration-300 translate-x-full lg:hidden" style="display:none;">
            ...
            <a class="text-base font-semibold hover:text-primary transition-colors" href="{{ route('school.ppdb', ['school' => 'sd']) }}">PPDB 2024</a>
        </div>
        <script>
            // Mobile menu toggle
    ...
    <footer class="mt-16 py-8 bg-charcoal text-white text-center">
        <div class="max-w-7xl mx-auto px-6">
            <span class="font-semibold">&copy; {{ date('Y') }} DIT Putra Pakuan. All rights reserved.</span>
        </div>
    </footer>
    @stack('scripts')
</body>
