...
<body style="margin:0; padding:0;" class="m-0 p-0 bg-background-light dark:bg-background-dark text-charcoal dark:text-slate-100"
    ...
    <nav id="school-nav" style="margin-top:0;" class="fixed top-0 z-50 w-full bg-charcoal text-white border-b border-primary/20 transition-all duration-300">
        <div class="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <a href="{{ route('school.home', ['school'=> 'smk']) }}">
                    <div class="size-10">
                        <img src="{{ asset('images/logo-yayasan.png') }}" alt="Logo SMK Putra Pakuan">
                    </div>
                </a>
                <div>
            ...
            <div class="hidden lg:flex items-center gap-6">
                <a class="text-xs font-semibold hover:text-primary transition-colors" href="{{ route('school.home', ['school' => 'smk']) }}">BERANDA</a>
                ...
                <a class="text-xs font-semibold hover:text-primary transition-colors" href="{{ route('school.jurusan.index', ['school' => 'smk']) }}">JURUSAN</a>
        ...
        <div id="mobile-menu" class="fixed inset-0 z-50 bg-charcoal/95 text-white flex flex-col gap-6 px-8 py-10 transition-all duration-300 translate-x-full lg:hidden" style="display:none;">
            ...
            <a class="text-base font-semibold hover:text-primary transition-colors" href="{{ route('school.home', ['school' => 'smk']) }}">BERANDA</a>
            ...
            <a class="text-base font-semibold hover:text-primary transition-colors" href="{{ route('school.jurusan.index', ['school' => 'smk']) }}">JURUSAN</a>
            ...
            @if($ppdbLive)
            <a class="text-base font-semibold hover:text-primary transition-colors" href="{{ route('school.ppdb', ['school' => 'smk']) }}">{{ $ppdbLabel }}</a>
            @endif
        </div>
        <script>
            // HTMX-safe mobile menu handlers
            function initSmkMobileMenu() {
                ...
                if (mobileDirektoriToggle && mobileDirektoriDropdown && !mobileDirektoriToggle._htmxInit) {
                    ...
                    if (!document._smkDirektoriOutsideInit) {
                        document._smkDirektoriOutsideInit = true;
            ...
            if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', initSmkMobileMenu);
            } else {
                initSmkMobileMenu();
            }

            document.addEventListener('htmx:afterSettle', initSmkMobileMenu);
        </script>
    </nav>
    <!-- End Navbar -->
    <script>
                ...
                <div class="col-span-1 lg:col-span-1 space-y-6">
                    <div class="flex items-center gap-3">
                        <div class="relative w-10 h-10 rounded-lg overflow-hidden border border-white/30">
                            <img src="{{ asset('images/logo-yayasan.png') }}" alt="SMK Putra Pakuan" class="w-full h-full object-cover" />
    ...
    <script>
        ...
        if (!window.__fabInitialized) {
            ...
            const initializeFabHandler = () => {
                ...
                if (smkFabButton) {
                    smkFabButton.addEventListener('click', function (e) {
                        e.stopPropagation();
